...
本清单用于验证辅助功能和麦克风权限归属于 Typeless，而非 Typeless Switch 或 Terminal。
...
1. 完全退出 Typeless。
2. 从 Typeless Switch 执行“连接”。
3. 在 Typeless Onboarding 点击粘贴权限的“允许”。
4. 记录系统弹窗显示的应用名称。
5. 在系统设置中允许后返回 Typeless。

历史实际结果：系统弹窗显示 Typeless Switch；Typeless 页面没有进入已授权状态。

## LaunchServices 最小实验

测试环境：macOS（请在最终验收记录中填写具体版本）。

1. 在「系统设置 → 隐私与安全性」的“辅助功能”和“麦克风”中，手动移除 Typeless Switch、
...
在固定路径安装本轮生成的 Release App 后，完全退出 Typeless 和 Typeless Switch。

1. 从 Toolkit 建立连接，在 Typeless Onboarding 申请辅助功能。
2. 确认弹窗和系统设置列表显示 Typeless，而不是 Typeless Switch 或 Terminal；允许后 Onboarding
...
辅助功能弹窗显示：Typeless / Typeless Switch / Terminal / 没有弹窗
允许后 Onboarding：已完成 / 仍停留在“允许”
麦克风弹窗显示：Typeless / Typeless Switch / Terminal / 没有弹窗
两项授权后：可继续 / 无法继续
重启后权限：保持有效 / 再次要求授权
