...
function publicRotationSettings(settings = {}) {
  return {
    enabled: settings.enabled === true,
    mode: settings.mode === 'auto' ? 'auto' : 'notify',
    word_threshold: Number.isSafeInteger(settings.word_threshold) ? settings.word_threshold : 2000,
    warning_words: Number.isSafeInteger(settings.warning_words) ? settings.warning_words : 100,
    interval_minutes: [10, 15, 30].includes(settings.interval_minutes) ? settings.interval_minutes : 15,
  };
}

function publicRotationIssue(issue) {
  if (!issue || typeof issue !== 'object') return null;
  return {
    code: String(issue.code || 'UNKNOWN'),
    message: String(issue.message || ''),
    action: String(issue.action || 'settings'),
    ...(typeof issue.account_id === 'string' && issue.account_id ? { account_id: issue.account_id } : {}),
    retryable: issue.retryable === true,
  };
}

function publicRotationStatus(status = {}) {
  return {
    phase: typeof status.phase === 'string' ? status.phase : 'disabled',
    message: String(status.message || ''),
    last_check_at: typeof status.last_check_at === 'string' ? status.last_check_at : null,
    next_check_at: typeof status.next_check_at === 'string' ? status.next_check_at : null,
    current_user_id: typeof status.current_user_id === 'string' ? status.current_user_id : null,
    used_words: Number.isFinite(status.used_words) ? status.used_words : null,
    last_result: typeof status.last_result === 'string' ? status.last_result : null,
    issue: publicRotationIssue(status.issue),
    candidate_issues: Array.isArray(status.candidate_issues)
      ? status.candidate_issues.map(publicRotationIssue).filter(Boolean)
      : [],
    notification_error: typeof status.notification_error === 'string' ? status.notification_error : null,
  };
}
