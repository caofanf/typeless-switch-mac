...
async function captureTokenCDP(port, autoRestart = true) {
  ...
  return withCDP(async (_send, ev) => {
    const current = await ev(`(async () => {
      if (!window.ipcRenderer || !window.ipcRenderer.invoke) return null;
    ...
    if (!current?.user_id || !current.refresh_token) {
      throw new Error('未读取到有效登录凭证,请先在 Typeless 登录账号');
    }
    ...
    const meta = accountMetaFromUserInfo(user_info, user_id);
    if (!meta.email && current.email) meta.email = current.email;
    if (meta.nickname === (user_id || '').slice(0, 8) && (meta.email || current.email)) {
      meta.nickname = meta.email || current.email;
    }
    return { token, origin, user_id, user_info, ...meta, captured_at: new Date().toISOString() };
  }, usePort);
}
