...
## native-macos-v2.0.0 - 2026-07-14

### Changed

- 将“账号”设为默认首页，并将原生侧栏精简、重排为账号、主词库、备份与恢复、概览、设置。
- 将“概览”下调到设置上方，移除诊断和高级工具的独立入口；概览中的版本提醒会直接定位到设置页的高级功能区域。
- 统一设置页与其他内容页的系统语义背景、内容宽度和间距，隐藏最近活动、诊断日志与辅助功能状态。
- 将版本漂移、Typeless 补丁、设备标识重置、数据目录和高级任务完整迁入设置页下方的“高级功能”。
- 全新安装默认不显示系统菜单栏状态图标；已经保存过的用户选择保持不变，仍可在设置中手动开启原有菜单。
- 将原生 App 版本升级到 `2.0.0`（构建号 `2`），Sidecar 核心和协议版本继续保持 `1.0.0`。

### Tests

- 新增默认首页、侧栏顺序、设置页定位以及菜单栏默认值与偏好持久化回归测试。
